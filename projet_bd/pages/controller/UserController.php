...
class UserController extends ControllerBase
{
    ...
    {
        $user = $_SESSION;
        $tweets = $this->app->getService('TweetFinder')->findTweetByUser($_SESSION['id']);
        $retweets_id = $this->app->getService('RetweetFinder')->findRetweetByUser($_SESSION['id']);

        foreach ($retweets_id as $retweet_id)
        {
            $tweets = $this->app->getService('TweetFinder')->findTweetById($retweet_id["tweet_id"]);
        }
